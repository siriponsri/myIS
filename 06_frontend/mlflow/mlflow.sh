...
usage() {
  printf '%s\n' \
    "Usage: bash 06_frontend/mlflow/mlflow.sh {doctor|bootstrap|start|status|url}" \
    "" \
    "Environment:" \
...
case "$command_name" in
  ...
  bootstrap)
    ...
    MYIS_MLFLOW_STORE="$STORE_ROOT" uv run --no-sync python "$REPOSITORY_ROOT/05_code/scripts/bootstrap_mlflow.py"
    run_app doctor
    ;;
  start)
