...
const GATE_LABELS = {
  D1_START_CAMPAIGN: "Start campaign",
  D2_OPEN_FINAL: "Open final evaluation",
  D3_SUBMIT_RELEASE: "Submit or release",
};
...
const state = {
  ...
  activePhaseId: "F0",
  activeTaskId: "F0.1",
  activeGateId: "D1_START_CAMPAIGN",
...
async function refreshAll() {
  ...
  try {
    ...
    applyResult(results[13], (value) => { state.readModel = value; });
...
function renderReadModel() {
  ...
  elements.overview_readiness.replaceChildren(
    el("div", "readiness-summary",
      el("div", "readiness-status", readiness.status === "ready" ? "Publication ready" : "Publication blocked"),
      el("p", "lede", `${campaign.title || campaign.campaign_id || "SCOPE campaign"} · ${campaign.status || "preparation"}`),
      el("dl", "readiness-facts",
        el("div", "fact", el("dt", "", "Runs"), el("dd", "", String(runCount))),
        el("div", "fact", el("dt", "", "Metrics"), el("dd", "", String(metricCount))),
        el("div", "fact", el("dt", "", "Cost"), el("dd", "", cost.actual == null ? "not measured" : `${cost.actual} ${cost.currency || "USD"}`)),
      ),
    ),
  );
  const projection = elements.overview_projections;
  projection.replaceChildren(el("div", "projection-health-card",
    el("p", "section-label", "Canonical projection"),
    el("p", "", `read-model.v1 · ${shortHash(model.projection_revision || "")}`),
    el("p", "detail-thai", "Dashboard เป็น projection จาก Research ไม่ใช่แหล่งข้อมูลหลัก"),
  ));
}
...
function renderPresentation() {
  ...
  const model = state.readModel;
  const facts = model ? el("section", "presentation-facts",
    el("p", "section-label", "Live campaign projection"),
    el("h2", "", model.campaigns?.[0]?.title || "SCOPE / AutoIndex"),
    el("div", "presentation-fact-grid",
      el("div", "fact", el("span", "", "Phase"), el("strong", "", model.campaigns?.[0]?.status || "preparation")),
      el("div", "fact", el("span", "", "Experiments"), el("strong", "", String(model.experiments?.length || 0))),
      el("div", "fact", el("span", "", "Evidence"), el("strong", "", String(model.evidence?.length || 0))),
      el("div", "fact", el("span", "", "Readiness"), el("strong", "", model.publication_readiness?.status || "blocked")),
    ),
  ) : null;
