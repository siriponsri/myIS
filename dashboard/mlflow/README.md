...
bash dashboard/mlflow/mlflow.sh doctor
bash dashboard/mlflow/mlflow.sh start
```
...
```bash
bash dashboard/mlflow/mlflow.sh bootstrap
```
...
`control/campaigns/scope-autoindex-v1.yaml`. Validate the campaign contract
without writing:

```powershell
uv run --no-sync python scripts/mlflow_doctor.py --repository-root . --store-root "$MYIS_MLFLOW_STORE"
```
