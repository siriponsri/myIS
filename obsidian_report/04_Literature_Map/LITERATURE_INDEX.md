---
schema_version: "myis.obsidian-note.v2"
read_model_revision: "d2b84e42b519465cad663a9f1ee02a0c61fdbc474f330bbbbb8f75721e7c1f4c"
read_model_sha256: "06f7bd023dcc014f8d377c53d7229467c57d02cfce6f6dd07f00efb25709e9f4"
source_commit: "f3dd8a2693406647a8cfd97f93e38b302e00a95f"
projection_schema_version: "myis.integrated-projection.v2"
source_run_ids: []
source_manifest_sha256: []
related_literature_ids: []
related_decision_ids: ["D2_OPEN_FINAL","D3_SUBMIT_RELEASE"]
evidence_class: "engineering"
scientific_authority: false
claim_boundary: "engineering_provenance_only"
generated_from_revision: "d2b84e42b519465cad663a9f1ee02a0c61fdbc474f330bbbbb8f75721e7c1f4c"
last_material_update: "2026-08-03T15:12:00Z"
next_authorized_action: "Owner-local P2 measured preflight"
managed_by: "myis-report"
edit_policy: "generated_do_not_edit"
safe_to_present: true
created_at: "2026-08-03T15:12:00Z"
updated_at: "2026-08-03T15:12:00Z"
note_id: "LITERATURE-INDEX"
note_type: "literature_synthesis"
phase_id: "P0_FOUNDATION"
task_id: "P0.3"
workflow_status: "in_progress"
evidence_maturity: "historical_exposed"
claim_level: "descriptive"
paper_count: 154
---

# Literature Map

- [[U001]] - PatentMatch: A Dataset for Matching Patent Claims & Prior Art
- [[U002]] - Prior Art Search and Reranking for Generated Patent Text
- [[U003]] - Artificial intelligence for patent prior art searching
- [[U004]] - Patent Prior Art Search using Deep Learning Language Model
- [[U005]] - PatentSBERTa: A Deep NLP based Hybrid Model for Patent Distance and Classification using Augmented SBERT
- [[U006]] - A Combination of BERT and BM25 for Patent Search
- [[U007]] - CoPatE: A Novel Contrastive Learning Framework for Patent Embeddings
- [[U008]] - SEARCHFORMER: Semantic patent embeddings by siamese transformers for prior art search
- [[U009]] - PaECTER: Patent-level Representation Learning using Citation-informed Transformers
- [[U010]] - Enhancing the Patent Matching Capability of Large Language Models via the Memory Graph
- [[U011]] - DAPFAM: A Domain-Aware Family-level Dataset to benchmark cross-domain patent retrieval
- [[U012]] - PatenTEB: A Comprehensive Benchmark and Model Family for Patent Text Embedding
- [[U013]] - Patent Representation Learning via Self-supervision
- [[U014]] - Rethinking patent retrieval with language models: Toward scalable and efficient search
- [[U015]] - Patent Retrieval with Few-Shot Fine-Tuning and Quantized Embeddings
- [[U016]] - BMRETRIEVER: Tuning Large Language Models as Better Biomedical Text Retrievers
- [[U017]] - Needle in a haystack: Harnessing AI in drug patent searches and prediction
- [[U018]] - Enhancing Patent Retrieval using Text and Knowledge Graph Embeddings: A Technical Note
- [[U019]] - PatentBERT: Patent Classification by Fine-Tuning BERT Language Model
- [[U020]] - Zero-shot Hybrid Retrieval and Reranking Models for Biomedical Literature
- [[U021]] - PatCID: an open-access dataset of chemical structures in patent documents
- [[U022]] - Enhancing patent retrieval using automated patent summarization
- [[U023]] - LLM-powered Real-time Patent Citation Recommendation for Financial Technologies
- [[U024]] - EvoPat: A Multi-LLM-Based Patents Summarization and Analysis Agent
- [[U025]] - Deep learning for patent landscaping using transformer and graph embedding
- [[U026]] - MedCPT: Contrastive Pre-trained Transformers with Large-scale PubMed Search Logs for Zero-shot Biomedical Information Retrieval
- [[U027]] - Efficient Patent Searching Using Graph Transformers
- [[U028]] - Contrastive learning enhanced retrieval-augmented few-shot framework for multi-label patent classification
- [[U029]] - CLEF-IP 2011: Retrieval in the Intellectual Property Domain
- [[U030]] - CLEF-IP 2012: Retrieval Experiments in the Intellectual Property Domain
- [[U031]] - Report on the CLEF-IP 2011 Experiments: Exploring Patent Summarization
- [[U032]] - A comparative analysis of embedding models for patent similarity
- [[U033]] - A survey on automated and AI-based tools for patent retrieval with a special focus on the life sciences domain
- [[U034]] - A Survey on Patent Analysis: From NLP to Multimodal AI
- [[U035]] - BEIR: A Heterogeneous Benchmark for Zero-shot Evaluation of Information Retrieval Models
- [[U036]] - Towards Automated Patent Workflows: AI-Orchestrated Multi-Agent Framework for Intellectual Property Management and Analysis (PatExpert)
- [[U037]] - ColBERTv2: Effective and Efficient Retrieval via Lightweight Late Interaction
- [[U038]] - H-ProtoRAG: A hierarchical prototype-based retrieval-augmented framework for multilingual CPC patent classification and prior-art retrieval
- [[U039]] - FullRecall: A Semantic Search-Based Ranking Approach for Maximizing Recall in Patent Retrieval
- [[U040]] - Mining patents with large language models elucidates the chemical function landscape
- [[U041]] - Natural Language Processing (NLP) tools for Pharmaceutical Manufacturing Information Extraction from Patents
- [[U042]] - Toward Knowledge-Based System for African Traditional Herbal Medicine using NLP
- [[U043]] - PatentSemTech 2024: Improving Patent Classification using AI-Generated Summaries
- [[U044]] - ChatGPT vs. Google Gemini: Assessing AI Frontiers for Patent Prior Art Search Using European Search Reports
- [[U045]] - Intermediate Domain Alignment for Patent-Product Image Retrieval (IDAMA)
- [[U046]] - Large Language Model Informed Patent Image Retrieval (Multimodal Distribution-Aware)
- [[U047]] - Prior Art Retrieval using Patent Sections (CLEF-IP 2010 – Dhondt et al.)
- [[U048]] - Hierarchical Multi-Positive Contrastive Learning for Patent Image Retrieval (DeepPatent2)
- [[U049]] - Citation-Driven Multi-View Training for Patent Embeddings: QaECTER and Sophia-Bench
- [[U050]] - Patent Language Model Pretraining with ModernBERT
- [[U051]] - Towards efficient patent analysis: A large language model and BERT-refined methodology for keyphrase extraction
- [[U052]] - Standard Applicability Judgment and Cross-jurisdictional Reasoning: A RAG-based Framework for Medical Device Compliance
- [[U053]] - Intelligent Chinese patent medicine (CPM) recommendation framework: Integrating large language models, retrieval-augmented generation, and the largest CPM dataset
- [[U054]] - Research on the Global Patent Trend of Artificial Intelligence Medical Device
- [[U055]] - Improving Retrieval-Augmented Generation in Medicine with Iterative Follow-up Questions
- [[U056]] - An automatic patent literature retrieval system based on LLM-RAG
- [[U057]] - PAI-NET: Retrieval Augmented Generation Patent Network using Prior Art Information
- [[U058]] - GEPA: Reflective Prompt Evolution Can Outperform Reinforcement Learning
- [[U059]] - Is ChatGPT Good at Search? Investigating Large Language Models as Re-Ranking Agents
- [[U060]] - RankZephyr: Effective and Robust Zero-Shot Listwise Reranking is a Breeze!
- [[U061]] - FIRST faster listwise reranking with single token decoding
- [[U062]] - Rank-DistiLLM cross-encoder distillation
- [[U063]] - Rank-without-GPT: Building GPT-Independent Listwise Rerankers on Open-Source Large Language Models
- [[U064]] - Guiding Retrieval with LLM Listwise Rankers
- [[U065]] - APE Automatic Prompt Engineer
- [[U066]] - TextGrad automatic differentiation via text
- [[U067]] - Prompt Optimization is a Coin Flip
- [[U068]] - RAPO Retrieval-Augmented Prompt Optimization
- [[U069]] - A novel re-ranking architecture for patent search
- [[U070]] - POSIX: A Prompt Sensitivity Index For Large Language Models
- [[U071]] - OPTIMAS: Optimizing Compound AI Systems with Globally Aligned Local Rewards
- [[U072]] - Compound AI Systems Optimization: A Survey of Methods Challenges and Future Directions
- [[U073]] - DSPy: Compiling Declarative Language Model Calls into Self-Improving Pipelines
- [[U074]] - A Gentle Introduction to Conformal Prediction and Distribution-Free Uncertainty Quantification
- [[U075]] - Algorithmic Learning in a Random World
- [[U076]] - CAPO: Cost-Aware Prompt Optimization
- [[U077]] - Is It Novel and Why? Fine-Grained Patent Novelty Prediction Based on Passage Retrieval
- [[U078]] - Benchmarking Patent Embeddings: A Multi-Task Evaluation of 22 Models Across Retrieval, Classification, and Clustering
- [[U079]] - PEEM: Prompt Engineering Evaluation Metrics for Interpretable Joint Evaluation of Prompts and Responses
- [[U080]] - AIPO: Automatic Instruction Prompt Optimization by Model Itself with Gradient Ascent
- [[U081]] - ResearchRubrics: A Benchmark of Prompts and Rubrics for Evaluating Deep Research Agents
- [[U082]] - SkillOpt: Executive Strategy for Self-Evolving Agent Skills
- [[U083]] - BEIR: A Heterogeneous Benchmark for Zero-shot Evaluation of Information Retrieval Models
- [[U084]] - ColBERTv2: Effective and Efficient Retrieval via Lightweight Late Interaction
- [[U085]] - Precise Zero-Shot Dense Retrieval without Relevance Labels (HyDE)
- [[U086]] - M3-Embedding: Multi-Linguality, Multi-Functionality, Multi-Granularity Text Embeddings (BGE-M3)
- [[U087]] - Query Rewriting for Retrieval-Augmented Large Language Models
- [[U088]] - Reciprocal Rank Fusion Based Hybrid Dense–Sparse Information Retrieval (RRF Hybrid)
- [[U089]] - NV-Embed: Improved Techniques for Training LLMs as Generalist Embedding Models
- [[U090]] - Jina-ColBERT-v2: A General-Purpose Multilingual Late Interaction Retriever
- [[U091]] - RAPTOR: Recursive Abstractive Processing for Tree-Organized Retrieval
- [[U092]] - From Local to Global: A GraphRAG Approach to Query-Focused Summarization
- [[U093]] - Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG
- [[U094]] - Interact-RAG: Elevating Retrieval-Augmented Generation via Active Corpus Interaction
- [[U095]] - MedGraphRAG: Graph-based Retrieval-Augmented Generation for Medical LLMs
- [[U096]] - PubMed Knowledge Graph 2.0: Connecting Papers, Patents, and Clinical Trials
- [[U097]] - KGEIR: Knowledge Graph-Enhanced Iterative Reasoning for Multi-Hop QA
- [[U098]] - Named Entity Recognition of Pharmacokinetic Parameters in Pharmaceutical Literature
- [[U099]] - Benchmarking LLM Faithfulness in RAG with Evolving Leaderboards (FaithJudge/HHEM)
- [[U100]] - HalluGraph: Auditable Hallucination Detection for Legal RAG via Knowledge Graph Alignment
- [[U101]] - Typhoon: Thai Large Language Models (7B)
- [[U102]] - NitiBench: A Comprehensive Study of LLM Framework Capabilities for Thai Legal QA
- [[U103]] - Survey on Thai NLP Language Resources and Tools
- [[U104]] - Agent-Mediated Dialogic Framework for Query Expansion (AMD: Socratic+Dialogic+Reflective)
- [[U105]] - DMQR-RAG: Diverse Multi-Query Rewriting for Retrieval-Augmented Generation
- [[U106]] - Collab-RAG: Boosting Retrieval-Augmented Generation for Complex Question Answering
- [[U107]] - ROSERAG: Robust Retrieval-Augmented Generation with Small-Scale LLMs via Diverse Augmentation
- [[U108]] - Qwen3 Technical Report
- [[U109]] - FinReflectKG-MultiHop: Financial QA Benchmark for Reasoning with KG Evidence
- [[U110]] - ICD-10-PCS Official Guidelines for Coding and Reporting 2023
- [[U111]] - Large Language Models Are Better Reasoners with Self-Verification
- [[U112]] - Recent Advances in Named Entity Recognition: A Comprehensive Survey
- [[U113]] - CLIRudit: Cross-Lingual Information Retrieval of Scientific Documents
- [[U114]] - New Artificial Intelligence Functionality in PE2E Search
- [[U115]] - Trademark Guide in Thailand
- [[U116]] - Diverse Augmentation from Large Language Models to Smaller Models for Efficient Retrieval (ACL 2025)
- [[U117]] - Tree-Based Text Retrieval via Hierarchical Clustering in RAG Frameworks: Application on Taiwanese Regulations
- [[U118]] - Graph RAG for Legal Norms: A Hierarchical and Temporal Approach
- [[U119]] - Optimizing Instructions and Demonstrations for Multi-Stage Language Model Programs
- [[U120]] - Efficient Memory Management for Large Language Model Serving with PagedAttention
- [[U121]] - AWQ: Activation-Aware Weight Quantization for On-Device LLM Compression and Acceleration
- [[U122]] - DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models
- [[U123]] - OpenThaiGPT 1.5: A Thai-Centric Open Source LLM
- [[U124]] - SeaLLMs: Large Language Models for Southeast Asia
- [[U125]] - SeaLLMs 3: Open Foundation Multilingual LLM
- [[U126]] - SEA-LION: Southeast Asian Languages in One Network
- [[U127]] - AttaCut: A Fast and Accurate Neural Thai Word Segmenter
- [[U128]] - MIRACL: Multilingual Information Retrieval Across 18 Languages
- [[U129]] - NeuCLIRBench: Modern Evaluation for Cross-Language IR
- [[U130]] - Efficiently Aligned Cross-Lingual Transfer via Prompt-Tuning (EACL 2024)
- [[U131]] - A Hybrid LLM–KG Framework for Biomedical QA (LLaMA-3B + Neo4j)
- [[U132]] - Judging the Judges: LLM-Generated Relevance Judgments (LLMJudge@SIGIR2024)
- [[U133]] - TREC 2024 RAG Track: Human vs LLM Support Judges
- [[U134]] - FaithJudge: Benchmarking LLM Faithfulness in RAG
- [[U135]] - BRIDGE: Uncovering Missing Relevant Chunks (ICLR 2026)
- [[U136]] - LegalBench-RAG: Benchmark for RAG in Legal Domain
- [[U137]] - A Reasoning-Focused Legal Retrieval Benchmark (CSLAW 2025)
- [[U138]] - Using the Triangle Inequality to Accelerate k-Means
- [[U139]] - Soft Language Prompts for Language Transfer (EACL 2024)
- [[U140]] - Hybrid Batch Training for Multilingual Retrieval (MIRACL)
- [[U141]] - Survey of Tasks, Datasets, Models in Legal NLP
- [[U142]] - TREC 2023 NeuCLIR Track: Cross-Language Neural Retrieval
- [[U143]] - Beyond Keywords: Optimizing Legal Information Retrieval through Embeddings, Cross-Encoders, and Large Language Models
- [[U144]] - Case-Aware LLM-as-a-Judge for Enterprise RAG
- [[U145]] - LegalBench: Measuring Legal Reasoning in LLMs
- [[U146]] - Medical Graph RAG: Towards Safe Medical Large Language Models via Graph Retrieval-Augmented Generation
- [[U147]] - Drug Repurposing via Graph-of-Thoughts (GoT) + PrimeKG
- [[U148]] - LLaDR: LLM-Assisted Biomedical Concept Mapping for Drug Repurposing
- [[U149]] - Benchmarking Patent Embeddings: A Multi-Task Evaluation of 22 Models Across Retrieval, Classification, and Clustering
- [[U150]] - The Structure of an Academic Paper
- [[U151]] - SkillOpt-Lite: Better and Faster Agent Self-Evolution via One Line of Vibe
- [[U152]] - Marginal Advantage Accumulation for Memory-Driven Agent Self-Evolution
- [[U153]] - SkillGrad: Optimizing Agent Skills Like Gradient Descent
- [[U154]] - AutoIndex: Learning Representation Programs for Retrieval
