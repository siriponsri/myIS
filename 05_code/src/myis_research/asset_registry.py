...
REGISTRY_RELATIVE_PATH = Path("00_governance/config/reusable_assets.yaml")
MAP_RELATIVE_PATH = Path("00_governance/REUSABLE_ASSET_MAP.md")
...
def validate_registry(payload: dict[str, Any], root: Path) -> None:
    ...
    tasks = parse_plan_tasks(root / PLAN_RELATIVE_PATH)
...
def render_asset_map(registry: dict[str, Any], root: Path) -> str:
    ...
    lines = [
        "# Reusable Asset Map",
        "",
        f"> Generated deterministically from {_TICK}00_governance/config/reusable_assets.yaml{_TICK}.",
