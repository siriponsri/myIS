...
PROTOCOL_FAMILY_ID = "scope-autoindex-v1"
PROJECT_CONFIG = Path("control/program.yaml")
...
def load_research_identity(repository_root: Path) -> ResearchIdentity:
    ...
    if not isinstance(document, dict) or not isinstance(document.get("project"), dict):
        raise IdentityValidationError("control/program.yaml must contain a project mapping")
    project = dict(document["project"])
    project.setdefault("version_class", VERSION_CLASS)
    project.setdefault("protocol_family_id", PROTOCOL_FAMILY_ID)
    project.setdefault("legacy_aliases", ["myIS", "CrossRoute", "SkillOpt"])
    return ResearchIdentity.from_mapping(project)
