...
def build_dashboard_snapshot(repository_root: Path) -> dict[str, Any]:
    ...
    decisions = _active_governance_decisions(
        repository_root / "control/decisions/records", plan
    )
    governance = _governance_states(plan, decisions)
    evidence_root = repository_root / "campaigns/scope-autoindex-v1/evidence"
    phase_views = []
...
def _approved_g0_exists(repository_root: Path) -> bool:
    ...
    approval_root = repository_root / "control" / "decisions" / "records"
