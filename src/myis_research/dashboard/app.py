...
def create_app(
    *,
    ...
    approvals = ImmutableJsonLedger(repository_root / "control/decisions/records", prior_field="prior_record_hash")
    private_root = repository_root / "projections/dashboard-private"
    ...
    def owner_gates(_: tuple[str, Any] = Depends(require_session)) -> dict[str, Any]:
        return _projection_response(lambda: {
            "schema_version": "myis.decisions.v1",
            "owner_decisions": ["D1_START_CAMPAIGN", "D2_OPEN_FINAL", "D3_SUBMIT_RELEASE"],
            "decisions": [payload for payload in build_read_model(repository_root).get("decisions", []) if payload.get("decision_id") in {"D1_START_CAMPAIGN", "D2_OPEN_FINAL", "D3_SUBMIT_RELEASE"}],
            "micro_gates": False,
        })
    ...
    def research_read_model(_: tuple[str, Any] = Depends(require_session)) -> dict[str, Any]:
        return _projection_response(lambda: build_read_model(repository_root))
    ...
    def presentation_topic_catalog(_: tuple[str, Any] = Depends(require_session)) -> dict[str, Any]:
        return _projection_response(lambda: _presentation_projection(repository_root))
...
def _frontend_file(repository_root: Path, relative_path: str) -> Path | None:
    ...
    if relative_path not in allowed:
        return None
    frontend_root = repository_root / "dashboard"
...
def _presentation_projection(repository_root: Path) -> dict[str, Any]:
    model = build_read_model(repository_root)
    campaign = model.get("campaigns", [{}])[0]
    readiness = model.get("publication_readiness", {})
    return {
        "schema_version": "myis.presentation-projection.v1",
        "source": "projections/read-model/read-model.v1.json",
        "topics": [{
            "topic_id": "scope-autoindex",
            "title": campaign.get("title", "SCOPE / AutoIndex"),
            "subtitle_en": "Research status generated from the canonical read model",
            "subtitle_th": "สถานะงานวิจัยจาก read model กลาง",
            "sections": [
                {"section_id": "status", "title_en": "Current status", "title_th": "สถานะปัจจุบัน", "body_en": str(campaign.get("status", "preparation")), "body_th": "สถานะอยู่ในช่วงเตรียมการ"},
                {"section_id": "readiness", "title_en": "Publication readiness", "title_th": "ความพร้อมตีพิมพ์", "body_en": str(readiness.get("status", "blocked")), "body_th": "ยังต้องปิดหลักฐานและตรวจ readiness ตามรายการ"},
            ],
        }],
    }
