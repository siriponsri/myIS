...
param(
    ...
    [ValidatePattern('^/opt/myis/[A-Za-z0-9._/-]+$')] [string]$RemoteRoot = '/opt/myis/a1.2-v8',
    [ValidatePattern('^/opt/myis/[A-Za-z0-9._/-]+$')] [string]$SourceRemoteRoot = '/opt/myis/a1.2-v7',
...
switch ($Action) {
    'stage-repair' {
        ...
        $extract = "set -eu; archive=${RemoteRoot}/incoming/frozen-code-bundle.tar.gz; actual=`$(sha256sum `"`$archive`" | awk '{print `$1}'); test `"`$actual`" = '$ExpectedBundleSha256'; tar -tzf `"`$archive`" | awk 'BEGIN{ok=1} /^\//{ok=0} /(^|\/)\.\.(`$|\/)/{ok=0} END{exit ok?0:1}'; tar -tvzf `"`$archive`" | awk 'substr(`$1,1,1) !~ /^-`$/ {exit 1}'; tar --no-same-owner --no-same-permissions -xzf `"`$archive`" -C ${RemoteRoot}/current; test -f ${RemoteRoot}/current/BUNDLE_MANIFEST.json"
    ...
    'start' {
        ...
        if ($ExpectedGitTree -notmatch '^[a-f0-9]{40,64}$') { throw 'ExpectedGitTree is required and invalid.' }
        if ($ExpectedBundleSha256 -notmatch '^[a-f0-9]{64}$') { throw 'ExpectedBundleSha256 is required and invalid.' }
        $launch = "HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PIP_NO_INDEX=1 PYTHONDONTWRITEBYTECODE=1 MYIS_REMOTE_MODE=a1_2_preflight_only PYTHONPATH=${RemoteRoot}/current/src ${RemoteRoot}/venv/bin/python -m myis_research.armindex.a1_2_live_preflight_packaging_v8 --repository-root ${RemoteRoot}/current validate-verification-marker --marker ${outputRoot}/preflight/verification-pass.v8.json --expected-commit $ExpectedGitCommit --expected-tree $ExpectedGitTree --expected-manifest-digest $ExpectedManifestDigest --expected-bundle-sha256 $ExpectedBundleSha256 && HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PIP_NO_INDEX=1 PYTHONDONTWRITEBYTECODE=1 MYIS_REMOTE_MODE=a1_2_preflight_only bash ${RemoteRoot}/current/scripts/a1_2_vast/remote-live-preflight-v6.sh ${RemoteRoot} ${ExpectedManifestDigest}"
...
$receipt = [ordered]@{
    schema_version = 'myis.armindex-a1.2-direct-base-coordinator.v8'
    action = $Action
    status = $(if ($DryRun) { 'dry_run_validated' } else { 'completed' })
    repair_lineage = 'v7_staged_artifacts_reused_with_validation_complete_bundle'
