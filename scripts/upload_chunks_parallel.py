...
def main() -> int:
    ...
    chunks = sorted((root / "bundle" / "a4x1" / "chunks").glob("part-*"))
    destination = "/opt/myis/a4-goal001-20260818T235614Z-a4x1/incoming/chunks"
    base = [
    ...
    def upload(path: Path) -> str:
        subprocess.run(
            base + [str(path), f"root@38.49.42.120:{destination}/{path.name}"],
            check=True,
            timeout=900,
        )
        return path.name
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        print(list(pool.map(upload, chunks)))
    return 0
