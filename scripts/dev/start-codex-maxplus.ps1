...
& $checker -ProfileName maxplus -ProfileRoot $ProfileRoot -WhatIf:$WhatIf
if ($WhatIf) { exit 0 }
$resolvedProfile = if ($ProfileRoot) { [IO.Path]::GetFullPath($ProfileRoot) } else { Join-Path $HOME '.codex-maxplus' }
...
try {
    ...
    Write-Output 'Starting Codex with the MaxPlus profile. Verify /status manually.'
    & codex @CodexArgs
    exit $LASTEXITCODE
}
finally {
