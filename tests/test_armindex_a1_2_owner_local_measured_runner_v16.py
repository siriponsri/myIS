...
def _manifest(tmp_path: Path) -> Path:
    ...
    corpus.write_text(
        "".join(
            json.dumps(
                {"logical_id": f"unit-{i}", "family_token": f"F-{i:032x}", "text": "alpha", "source_token_count": 1},
                separators=(",", ":"),
            )
            + "\n"
            for i in range(100)
...
def test_owner_local_bridge_rejects_gate_drift_and_writes_nothing(tmp_path: Path) -> None:
    ...
    with pytest.raises(OwnerLocalMeasuredRunnerV16Error, match="gates"):
        run_owner_local_measured_screen(manifest, output_root=tmp_path / "owner-receipts", batch_executor=lambda **_: {})
...
def test_owner_local_bridge_rejects_partial_executor_result(tmp_path: Path) -> None:
    ...
    with pytest.raises(OwnerLocalMeasuredRunnerV16Error, match="exactly 100"):
        run_owner_local_measured_screen(manifest, output_root=tmp_path / "owner-receipts", batch_executor=short_executor)
    assert not (tmp_path / "owner-receipts").exists()
