...
def _manifest(root: Path, archive: Path, *, tie: bool = False) -> Path:
    ...
    performance = {"compile_latency_ms": 1.0, "index_latency_ms": 2.0, "search_latency_ms": {"p50": 1.0, "p95": 2.0, "p99": 3.0}, "throughput_qps": 1.0, "wall_seconds": 4.0}
    ...
    for index, arm in enumerate(ARM_IDS):
        score = 1.0
        latency = 1.0 if tie else float(index + 1)
        promotions.append({"arm_id": arm, "recall_at_100_out": score, "ndcg_at_100_out": score, "latency_p95_ms": latency, "cost_per_query_usd": latency, "simplicity": latency})
    body = {"schema_version": "myis.armindex-a1.2-owner-local-evaluation-input.v16", "status": "READY", "safe_return_archive_sha256": file_sha256(archive), "qrels": {"path": qrels.name, "sha256": file_sha256(qrels), "count": 150}, "membership": {"path": membership.name, "sha256": file_sha256(membership), "count": 150}, "lineage_by_cell": {cell: lineage for cell in cells}, "cell_metrics": {cell: {"performance": performance, "resources": resources, "reliability": reliability} for cell in cells}, "promotion": {"rule": "lexicographic_recall_ndcg_latency_cost_simplicity", "max_arms": 3, "arms": promotions}}
