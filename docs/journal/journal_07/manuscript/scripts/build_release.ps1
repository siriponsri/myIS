...
$legacyReleaseFiles | Where-Object { Test-Path -LiteralPath $_ } | Remove-Item -Force
$env:MPLCONFIGDIR = Join-Path $env:TEMP "mplconfig-rcrs-v07"
...
try {
    ...
    Copy-Item -Force main.pdf (Join-Path $pdfDir "RCRS_WPI_MANUSCRIPT_V07_SUBMISSION.pdf")
    Copy-Item -Force reader_preview.pdf (Join-Path $pdfDir "RCRS_WPI_MANUSCRIPT_V07_READER_PREVIEW.pdf")
    Copy-Item -Force title_page.pdf (Join-Path $pdfDir "RCRS_WPI_TITLE_PAGE_PLACEHOLDER_V07.pdf")
    Copy-Item -Force figures\graphical_abstract.pdf (Join-Path $figureDir "RCRS_WPI_GRAPHICAL_ABSTRACT_V07.pdf")
    Copy-Item -Force figures\graphical_abstract.png (Join-Path $figureDir "RCRS_WPI_GRAPHICAL_ABSTRACT_V07.png")

    $releaseFiles = @(
        Join-Path $pdfDir "RCRS_WPI_MANUSCRIPT_V07_SUBMISSION.pdf"
        Join-Path $pdfDir "RCRS_WPI_MANUSCRIPT_V07_READER_PREVIEW.pdf"
        Join-Path $pdfDir "RCRS_WPI_TITLE_PAGE_PLACEHOLDER_V07.pdf"
        Join-Path $figureDir "RCRS_WPI_GRAPHICAL_ABSTRACT_V07.pdf"
        Join-Path $figureDir "RCRS_WPI_GRAPHICAL_ABSTRACT_V07.png"
    )
