...
This V0.7 package contains no query identifiers, patent/family identifiers,
...
`aggregate_ledger.json` is the only numeric input read by
`scripts/build_figures.py`. The table below records its canonical sources.
...
cohort aggregate with 17,429 relevant family-query pairs. A5 includes all
judged positive families for eligible queries; it is not filtered to strict
OUT relations. A6/A7 OUT is the
