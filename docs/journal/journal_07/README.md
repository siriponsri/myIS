...
- `output/pdf/RCRS_WPI_MANUSCRIPT_V07_READER_PREVIEW.pdf`: compact two-column
  reading copy built with the official Elsevier class. Its aggregate appendix
  starts on a fresh single-column page so the wide tables remain readable.
- `output/pdf/RCRS_WPI_MANUSCRIPT_V07_SUBMISSION.pdf`: single-column blinded
  submission manuscript.
- `output/pdf/RCRS_WPI_TITLE_PAGE_PLACEHOLDER_V07.pdf`: separate title-page
  placeholder that still requires owner-approved author details.
- `output/figures/RCRS_WPI_GRAPHICAL_ABSTRACT_V07.pdf`: vector master.
- `output/figures/RCRS_WPI_GRAPHICAL_ABSTRACT_V07.png`: 600-dpi copy.
...
- `manuscript/data/aggregate_ledger.json`: machine-readable aggregate figure inputs.
- `manuscript/data/final_system_programs.json`: serialized Final-872 system specifications.
- `manuscript/scripts/build_release.ps1`: complete V07 build and package copy.
...
preview, and title page, copies V07 release files, and writes SHA-256 hashes.

## Version workflow

For the next review, create `journal_08/` as a complete sibling copy of this
