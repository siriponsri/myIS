"""Build aggregate-safe artwork for the RCRS WPI V0.4 manuscript.

No benchmark records are read. Every plotted value is an aggregate copied
from the canonical artifacts listed in ../data/DATA_PROVENANCE.md. Vector PDF
is the submission master; 600 dpi PNG is supplied for inspection.
"""
...
def fig1_protocol():
    ...
    nodes = [
        ...
        (0.51, "Frozen retriever $a$", "weights and index fixed", BLUE),
    ...
    stages = [
        ...
        (0.51, "Selection-125", "one exposure", GREY),
    ...
    ax.text(0.50, 0.035, "System choice was fixed before Final-872; later analyses use the unchanged winner.", ha="center", va="bottom", fontsize=6.3, color=GREY, style="italic")
...
def fig2_transfer():
    transfer = np.array(
        [
            [0.41843575419, 0.337430167598, 0.362569832402],
            [0.418715083799, 0.341340782123, 0.359497206704],
            [0.419273743017, 0.338268156425, 0.360614525140],
        ]
    )
    controls = {
        "Best single": (0.41843575419, 0.347098229677, 0.290589053230),
        "Top-two RRF": (0.418715083799, 0.352747182825, 0.293715529655),
        "All-primary / top-three": (0.415083798883, 0.346250339835, 0.284772266617),
        "Commercial-only": (0.369273743017, 0.308967311892, 0.258115907613),
    }
    ...
    ax1.imshow(norm, cmap="Blues", vmin=0, vmax=1, aspect="equal")
    for i in range(3):
        for j in range(3):
            ax1.text(
                j,
    ...
    ax1.text(0.0, -0.29, "Shading normalized within each target column", transform=ax1.transAxes, fontsize=6.5, color=GREY)
    ...
    ax2.set_xlabel("Development score")
...
def fig3_final():
    metrics = ["Recall@100", "nDCG@100", "nDCG@10"]
    champion = np.array([0.4424757900, 0.3655952440, 0.2974593741])
    static = np.array([0.3310970948, 0.2792530035, 0.2336656011])
    deltas = np.array([0.1113786952, 0.0863422405, 0.0637937730])
    lower = np.array([0.1022935780, 0.0786733558, np.nan])
    upper = np.array([0.1204383282, 0.0940765598, np.nan])
    ...
    ax1.scatter(static, y, s=31, facecolors=WHITE, edgecolors=GREY, linewidths=1.1, marker="o", label="Static", zorder=3)
    ax1.scatter(champion, y, s=34, color=BLUE, marker="o", label="RCRS", zorder=3)
    for i in y:
        ax1.text(static[i] - 0.006, i - 0.13, f"{static[i]:.3f}", ha="right", fontsize=6.6, color=GREY)
        ax1.text(champion[i] + 0.006, i + 0.13, f"{champion[i]:.3f}", ha="left", fontsize=6.7, color=BLUE, weight="bold")
    ax1.set_yticks(y, metrics)
    ax1.set_ylim(2.55, -0.30)
    ax1.set_xlim(0.19, 0.49)
    ax1.set_xlabel("Final-872 aggregate score")
    title(ax1, "A", "Aggregate scores")
    grid(ax1, "x")
    ax1.legend(frameon=False, ncol=2, loc="upper center", bbox_to_anchor=(0.5, -0.20), handletextpad=0.35, columnspacing=1.0)
    ...
    for i in y:
        if np.isfinite(lower[i]):
            ...
            ax2.text(upper[i] + 0.004, i, f"+{deltas[i]:.4f}", va="center", fontsize=6.8, color=ORANGE, weight="bold")
        else:
            ax2.plot(deltas[i], i, marker="D", markerfacecolor=WHITE, markeredgecolor=GREY, markersize=4.5)
            ax2.text(deltas[i] + 0.004, i, f"+{deltas[i]:.4f}  descriptive", va="center", fontsize=6.4, color=GREY)
    ...
    for val, lab, color, marker, pos in zip(counts, labels, colors, markers, yy):
        ...
        ax3.text(val + 18, pos, f"{val}", va="center", fontsize=7.0, color=color, weight="bold")
...
def fig4_exposure():
    depths = np.array([10, 20, 50, 100, 200])
    pos = np.arange(len(depths))
    curves = {
        "ALL": np.array([0.136794974606, 0.214697496213, 0.336358371202, 0.438964626214, 0.546832397755]),
        "IN": np.array([0.187048067448, 0.277871602329, 0.413144709628, 0.528164111236, 0.645077432336]),
        "OUT": np.array([0.034022635484, 0.070933281792, 0.133993988468, 0.188449898653, 0.260166940437]),
    }
    fig, axes = plt.subplots(
        1,
        3,
    ...
    ax2.set_xlim(0, 4700)
    ax2.set_xlabel("Strict OUT-relevant pairs")
    title(ax2, "B", "Top-200 exposure")
    grid(ax2, "x")

    relation = ["ALL", "IN", "OUT"]
    observed = np.array([0.438964626214, 0.528164111236, 0.188449898653])
    oracle = np.array([0.546832397755, 0.645077432336, 0.260166940437])
    y = np.arange(3)
    for i in y:
...
def graphical_abstract():
    ...
    ax1.text(0.50, 0.07, "Retriever weights and index remain fixed", ha="center", fontsize=6.2, color=GREY)
    ...
    ax2.set_ylim(0, 1)
    ax2.set_yticks([])
    ax2.set_xlabel("OUT Recall@100 on Final-872")
    ...
    segments = [
        (796, "Top-100", BLUE, ""),
        (332, "Ranks 101-200", TEAL, "////"),
        (4065, "Absent from Top-200", MID, "...."),
    ]
    left = 0
