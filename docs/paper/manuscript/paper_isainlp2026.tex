...
reopening selection. Out of 5,193 out-of-domain relevant-family incidences,
4,065 were absent at rank 200. Perfect ordering of candidates already in the
pool could raise macro Recall@100 only from 0.188450 to 0.260167, an analytical
upper bound of 0.071717. The incidence counts and macro-Recall bound use
...
The central finding is visible in two quantities that must remain separate.
First, 4,065 of 5,193 out-of-domain relevant-family incidences were absent at
rank 200. This is a raw composition of relevance incidences. Second, an oracle
ordering of the same pool raised macro Recall@100 from 0.188450 to 0.260167,
leaving 0.071717 ordering headroom. The oracle is an upper bound, not a trained
reranker result. By binding both diagnostics to one post-confirmatory pool, the
study shows what each quantity can and cannot support.

This paper contributes a compact, receipt-bound case study of one selected
retriever on DAPFAM across confirmation, complete-corpus materialization, and
fixed-pool diagnosis. Within this deliberately narrow scope, raw candidate
availability and macro-averaged ordering headroom are reported together
without treating one as a decomposition of the other.
...
\caption{Two non-additive views of the frozen OUT family Top-200 pool. (a) Raw
relevant-family incidence counts sum to 5,193. (b) Macro Recall@100 is averaged
over the 905 receipt-defined OUT queries; the oracle reorders only candidates
already present at rank 200. The count composition and macro-Recall headroom
have different denominators and aggregation rules.}
\label{fig:diagnosis}
\end{figure*}
...
\subsection{Candidate Absence and Ordering Headroom Differ}
Figure~\ref{fig:diagnosis} presents the two views of the out-of-domain pool.
Among 5,193 relevant-family incidences, 796 were exposed by rank 100 and 332
first appeared at ranks 101--200. The remaining 4,065 were absent at rank 200.
Thus most raw out-of-domain relevance incidences were unavailable to any method
restricted to this candidate pool.
...
locates two boundaries without changing the selected system: candidate
availability in raw incidence units and ordering headroom in macro-Recall units.
...
improved over BM25 on 872 held-out queries. Yet its complete DAPFAM family pool
still exposed only 1,128 of 5,193 out-of-domain
relevant-family incidences by rank 200. Within that same pool, perfect ordering
could raise macro Recall@100 only from 0.188450 to 0.260167. Reporting these
quantities separately distinguishes candidate availability from ordering
headroom without turning an analytical bound into a system result. The
diagnostic therefore specifies whether a follow-up on this system should test
reordering or candidate expansion while leaving both interventions as open
questions.
