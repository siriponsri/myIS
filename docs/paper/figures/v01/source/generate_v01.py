...
def figure1(transfer: dict[tuple[str, str], float]) -> None:
    """A3: common absolute context plus matched-source deltas, row by row."""
    style()
    fig = plt.figure(figsize=(7.16, 2.26))
    gs = fig.add_gridspec(1, 2, width_ratios=(1.72, 1.0), left=0.145, right=0.982,
                          bottom=0.23, top=0.77, wspace=0.23)
    absolute = fig.add_subplot(gs[0, 0])
    delta = fig.add_subplot(gs[0, 1], sharey=absolute)
    ypos = {target: 2 - i for i, target in enumerate(TARGETS)}

    for target in TARGETS:
        y = ypos[target]
        values = [transfer[(source, target)] for source in SOURCES]
        matched = transfer[(target, target)]
        nominal_best = max(values)
        absolute.plot([min(values), max(values)], [y, y], color=SLATE, lw=2.2,
                      solid_capstyle="round", zorder=1)
        for source, value in zip(SOURCES, values):
            face = AMBER if value == nominal_best else SLATE
            absolute.scatter(value, y, s=42, marker=MARKERS[source], color=face,
                             edgecolor=WHITE, linewidth=0.75, zorder=3)
            if source == target:
                absolute.scatter(value, y, s=87, marker=MARKERS[source], facecolors="none",
                                 edgecolors=INK, linewidths=1.05, zorder=4)
            d = value - matched
            delta.scatter(d, y, s=42, marker=MARKERS[source], color=face,
                          edgecolor=WHITE, linewidth=0.75, zorder=3)
            if source == target:
                delta.scatter(d, y, s=87, marker=MARKERS[source], facecolors="none",
                              edgecolors=INK, linewidths=1.05, zorder=4)
        absolute.text(max(values) + 0.0006, y + 0.22, f"range {max(values) - min(values):.6f}",
                      color=SLATE_DARK, fontsize=6.9, ha="left", va="center")

    absolute.set_xlim(0.33, 0.425)
    absolute.set_ylim(-0.55, 2.55)
    absolute.set_yticks([2, 1, 0], ["PatEmbed target", "Arctic target", "Qwen3 target"])
    absolute.set_xticks([0.34, 0.38, 0.42], ["0.34", "0.38", "0.42"])
    absolute.set_xlabel("Recall@100 (common absolute scale)", fontsize=7.8, labelpad=4)
    clean_axis(absolute)
    panel_label(absolute, "A", "Absolute target score bands", "Three representation sources per consuming retriever")

    delta.axvline(0, color=INK, lw=0.9, zorder=1)
    delta.set_xlim(-0.0045, 0.0025)
    delta.set_xticks([-0.004, 0.0, 0.002], ["-0.004", "0", "+0.002"])
    delta.set_xlabel("Recall@100 difference from matched source", fontsize=7.8, labelpad=4)
    delta.set_yticks([2, 1, 0])
    delta.tick_params(labelleft=False)
    clean_axis(delta)
    panel_label(delta, "B", "Local transfer view", "Matched-source reference at zero")

    legend = [
        Line2D([0], [0], marker=MARKERS[source], linestyle="", markerfacecolor=SLATE,
               markeredgecolor=WHITE, markersize=5.4, label=SHORT[source])
        for source in SOURCES
    ]
    legend += [
        Line2D([0], [0], marker="o", linestyle="", markerfacecolor=AMBER,
               markeredgecolor=WHITE, markersize=5.4, label="nominal best"),
        Line2D([0], [0], marker="o", linestyle="", markerfacecolor="none",
               markeredgecolor=INK, markersize=7.2, label="matched pair"),
    ]
    fig.legend(handles=legend, ncol=5, frameon=False, loc="upper left", bbox_to_anchor=(0.145, 0.985),
               fontsize=7.0, columnspacing=1.0, handletextpad=0.35)
    fig.text(0.145, 0.045, "Small within-target ranges are descriptive development evidence; no significance inference is shown.",
             fontsize=7.2, color=SLATE_DARK)
...
def figure2(confirmation: dict[str, dict[str, float]], audit: dict[str, float]) -> None:
    ...
    style()
    fig = plt.figure(figsize=(7.16, 2.27))
    gs = fig.add_gridspec(2, 2, height_ratios=(3.0, 1.0), width_ratios=(1.04, 1.2),
                          left=0.11, right=0.985, top=0.78, bottom=0.15, hspace=1.18, wspace=0.27)
    absolute = fig.add_subplot(gs[0, 0])
    ...
    for metric, y in zip(metrics, ys):
        ...
        effect.text(r["paired_difference"], y + 0.21, f"+{r['paired_difference']:.6f}", ha="center", fontsize=8.1,
                    color=BLUE_DARK, fontweight="bold")
        effect.text(r["paired_difference"], y - 0.23, f"95% CI [{r['ci95_low']:.6f}, {r['ci95_high']:.6f}]",
                    ha="center", fontsize=6.75, color=SLATE_DARK)
    ...
    absolute.set_xlabel("Cross-domain score", fontsize=7.8, labelpad=3)
    clean_axis(absolute)
    panel_label(absolute, "A", "Final-872 absolute performance", "Gray comparator to blue selected complete system")
    ...
    effect.set_xlabel("Paired selected - comparator difference", fontsize=7.8, labelpad=3)
    clean_axis(effect)
    panel_label(effect, "B", "Paired effects with 95% CI", "The focal Recall@100 effect is +0.111379")
    ...
    ribbon.set_ylim(0, 1)
    ribbon.axis("off")
    ribbon.text(0.0, 1.15, "C", transform=ribbon.transAxes, color=BLUE_DARK, fontsize=9.2, fontweight="bold", va="bottom")
    ribbon.text(0.043, 1.15, "Recall wins / ties / losses", transform=ribbon.transAxes, color=INK,
                fontsize=9.2, fontweight="bold", va="bottom")
    ribbon.text(0.39, 1.15, "paired query outcomes (n=872)", transform=ribbon.transAxes, color=SLATE_DARK,
                fontsize=7.2, va="bottom")
    fig.text(0.11, 0.035, "Complete-system confirmation: the comparison does not identify a representation-only causal effect.",
             fontsize=7.05, color=SLATE_DARK)
...
def figure3(audit: dict[str, float]) -> None:
    ...
    gs = fig.add_gridspec(2, 1, height_ratios=(1.25, 1.0), left=0.10, right=0.985,
                          top=0.76, bottom=0.14, hspace=1.05)
    ...
    exposure.axis("off")
    exposure.text(0, 1.08, "A", color=BLUE_DARK, fontsize=9.2, fontweight="bold", va="bottom")
    exposure.text(240, 1.08, "Candidate exposure anatomy", color=INK, fontsize=9.2, fontweight="bold", va="bottom")
    exposure.text(1770, 1.08, "relevant-family incidences, not macro Recall components", color=SLATE_DARK,
                  fontsize=7.2, va="bottom")
    ...
    bound.text(observed, y + 0.16, f"Observed\n{observed:.6f}", ha="center", va="bottom", fontsize=7.1,
               color=BLUE_DARK, fontweight="bold")
    bound.text(oracle, y + 0.16, f"Perfect-ordering bound\n{oracle:.6f}", ha="center", va="bottom", fontsize=7.1,
               color=INK, fontweight="bold")
    bound.text((observed + oracle) / 2, y - 0.19, f"+{headroom:.6f} headroom", ha="center", va="center",
    ...
    clean_axis(bound)
    bound.text(0.0, 1.22, "B", transform=bound.transAxes, color=BLUE_DARK, fontsize=9.2, fontweight="bold", va="bottom")
    bound.text(0.045, 1.22, "What perfect ordering could recover", transform=bound.transAxes, color=INK,
               fontsize=9.2, fontweight="bold", va="bottom")
    bound.text(0.045, 1.07, "Analytical bound inside the immutable Top-200 pool; not reranker performance.", transform=bound.transAxes,
...
def overview() -> None:
    ...
    ax.axis("off")
    y = 0.47
    ax.add_patch(FancyArrowPatch((0.07, y), (0.485, y), arrowstyle="-|>", mutation_scale=9,
                                 linewidth=1.65, color=SLATE_DARK))
    ax.add_patch(FancyArrowPatch((0.515, y), (0.94, y), arrowstyle="-|>", mutation_scale=9,
                                 linewidth=1.65, color=BLUE_DARK))
    ...
    stages = [
        ...
        (0.64, "03", "FREEZE & CONFIRM", "Selection-125 -> Final-872", BLUE, "freeze"),
...
def main() -> None:
    ...
    figure3(audit)
    print("v01 figures generated after canonical A3/A5/A7 assertions passed")
